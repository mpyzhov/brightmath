    ...
    await ref
        ...
        .saveRootSession(
          RootSession(
            ...
            chapterId: _selectedChapter?.definition.id,
          ),
        );
  }

  Future<void> _restoreSession() async {
    final saved = await ref.read(sessionStoreProvider).loadRootSession();
    if (saved == null ||
        saved.pageIndex < 0 ||
        saved.pageIndex >= RootPage.values.length) {
      return;
    }

    final savedPage = RootPage.values[saved.pageIndex];
